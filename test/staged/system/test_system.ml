...
let moments (Value.P v) =
  ...
  let get i = match Value.dtype v with Dtype.F32 -> Value.get v i | Dtype.F64 -> Value.get v i | _ -> C.fail "dtype" in
...
let () =
  ...
  C.test "S8 Rng.u32 over 2^20 is bit-exact" (fun () ->
      let g = Graph.create ~name:"u32" ~outputs:(one "r" (Rng.u32 ~key:(scalar "seed" Dtype.I32) (vec (1 lsl 20)))) in
      diff g [ seed 12345l ]);
  C.test "S9 Rng.normal F32 on device: moments" (fun () ->
      let g = Graph.create ~name:"n" ~outputs:(one "z" (Rng.normal Dtype.F32 ~key:(scalar "seed" Dtype.I32) (vec (1 lsl 20)))) in
